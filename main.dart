...
class _HomePageState extends State<HomePage> {
  ...
  void trade(Market m, bool buy) {
    ...
    if (buy && cash < price) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Insufficient virtual funds'),
        ),
      );
      return;
    }
    ...
    setState(() {
      if (buy) {
        cash -= price;
        positions[m.name] = (positions[m.name] ?? 0) + 1;
      } else if ((positions[m.name] ?? 0) > 0) {
        cash += price;
        positions[m.name] = (positions[m.name] ?? 0) - 1;
      }
    });
  ...
  Widget marketPage() => ListView(
    padding: const EdgeInsets.all(16),
    children: [
      Card(
        child: ListTile(
          title: const Text('Virtual Balance'),
          subtitle: const Text('Virtual Trading Fund'),
          trailing: Text(
  ...
  Widget portfolioPage() => ListView(
    padding: const EdgeInsets.all(16),
    children: [
      Text(
        'Available Cash: ₹${cash.toStringAsFixed(2)}',
        style: const TextStyle(
          fontSize: 20,
          fontWeight: FontWeight.bold,
        ),
      ...
      ...positions.entries
          ...
          .map(
            (e) => ListTile(
              title: Text(e.key),
              trailing: Text('${e.value} unit(s)'),
            ),
          ),
      if (positions.values.every((v) => v == 0))
